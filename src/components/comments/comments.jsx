...
const Comments = () => {
  ...
  const [contents, changeContents, resetContents] = useLimitInput(30)
  ...
  return (
    <CmArea>
      <CommentsTitle className="fcc">
        <span>답글</span>
        <span><Button onClick={modalOpen}>등록</Button></span>
      </CommentsTitle>
      {commentsList.map((c) => {
      ...
      {pagingBtn.map((p, i) => {
        return <Button className="fcc" key={p + i} onClick={() => {setPage(p-1)}}>{p}</Button>
      })}
      </BtnArea>
      <Modal ref={modalRef}>
        <ModalInner>
          <p>작성자</p>
          <input onChange={changeAuthor} value={author} placeholder="10자 제한"></input>
        </ModalInner>
        <ModalInner>
          <p>댓글 내용</p>
          <textarea onChange={changeContents} value={contents} style={{height: "80px"}} placeholder="30자 제한"></textarea>
        </ModalInner>
        <BtnArea className="fcc">
